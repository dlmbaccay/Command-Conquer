...
// Game Validations 
bool validatePiece(bool aTurn, coordinates gamePiece, board gameBoard); // utilized by getPlayerPiece()
            ...
            case 'S':
                do
                {
                    ...
                    { 
                        ...
                        validPiece = validatePiece(aTurn, gamePiece, gameBoard);
                        if (validPiece == false) {
                            for (int i = 9; i < 12; i++)
                            {
                                gotoxy(36, i); printf("                            ");
...
coordinates getNextMove(bool aTurn, coordinates nextMove)
{
    ...
    gotoxy(36, 13); printf("\e[0;36m"); printf("SELECT TILE TO GO TO"); printf("\e[0m");
    do
    {
...
/*  validatePiece() : Function that check if the selected piece of the user is valid for movemement, returns true if the selected piece is valid
    ...
    @param gameBoard - the gameboard struct that contains all necessary variables used for game validations and follow the game behavior
*/
bool validatePiece(bool aTurn, coordinates gamePiece, board gameBoard)
{
    /* Variables */
    int i;
    bool valid = false;
